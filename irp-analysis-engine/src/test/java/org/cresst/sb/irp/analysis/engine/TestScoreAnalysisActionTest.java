...
public class TestScoreAnalysisActionTest {
	...
	public void whenTDSReport_HasValid_Scoring_ValidTestScoring_AndCellCategoryHasPassingValues() {
		...
		scores.add(new ScoreBuilder().measureOf("Overall").measureLabel("ThetaScore").value("-0.14785630410943")
				.standardError("0.457791032400169").toScore());
		scores.add(new ScoreBuilder().measureOf("1").measureLabel("ThetaScore").value("1.3335").standardError("0.831048294559457")
				.toScore());
		scores.add(new ScoreBuilder().measureOf("SOCK_2").measureLabel("ThetaScore").value("-1.2370123601497")
				.standardError("0.608838178456256").toScore());
		...
		CellCategory[] cellCategories =
				new CellCategory[] {
						new CellCategoryBuilder().correctDataType(true).acceptableValue(true).isFieldEmpty(false)
								...
								.tdsFieldNameValue(measureOf).toCellCategory(),

						new CellCategoryBuilder().correctDataType(true).acceptableValue(true).isFieldEmpty(false)
								.enumFieldCheckType(FieldCheckType.EnumFieldCheckType.P)
								.tdsFieldName(TestScoreAnalysisAction.EnumScoreFieldName.measureLabel.toString())
								.tdsFieldNameValue(measureLabel).toCellCategory(),

						new CellCategoryBuilder().correctDataType(true).acceptableValue(true).isFieldEmpty(false)
								.enumFieldCheckType(FieldCheckType.EnumFieldCheckType.P)
								.tdsFieldName(TestScoreAnalysisAction.EnumScoreFieldName.value.toString()).tdsFieldNameValue(value)
								.toCellCategory(),
	...
	private CellCategory[] createInvalidCellCategories() {
		CellCategory[] cellCategories =
				new CellCategory[] {
						...
						new CellCategoryBuilder().correctDataType(false).acceptableValue(false).isFieldEmpty(true)
								.enumFieldCheckType(FieldCheckType.EnumFieldCheckType.P)
								.tdsFieldName(TestScoreAnalysisAction.EnumScoreFieldName.value.toString()).tdsFieldNameValue("")
								.toCellCategory(),

						new CellCategoryBuilder().correctDataType(false).acceptableValue(false).isFieldEmpty(false)
								.enumFieldCheckType(FieldCheckType.EnumFieldCheckType.P)
								.tdsFieldName(TestScoreAnalysisAction.EnumScoreFieldName.standardError.toString())
